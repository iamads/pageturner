...
| D-019 | Serve configuration must start/reuse Tailscale, expose interactive HTTPS approval, and have bounded waits. | With the old script, Tailscale started successfully but Serve appeared stuck. Tailscale 1.102.4 source confirms first HTTPS use may print an admin URL and block awaiting capability approval; old output was redirected invisibly. | Configure script now shows reconnection progress, HTTPS approval URL, 120-second approval counter, and explicit success/failure/timeout; reset/status calls have 15-second bounds. The revised flow configured Serve successfully on-device. | User report and source inspection, 2026-09-22 |
| D-020 | HTTPS Certificates have been enabled for the tailnet through the admin console. | The owner completed the first-time HTTPS enablement after the old Serve command appeared to wait. | The machine FQDN can appear in public Certificate Transparency logs while endpoint access remains private. | User, 2026-09-22 |
| D-021 | Launch the persistent daemon with both the explicit state file and `--statedir=<extension>/state`. | The first iPhone TLS requests reached active Serve but handshakes failed with `no TailscaleVarRoot`. Tailscale 1.102.4 source requires a writable var root for managed certificate storage and does not infer one from this state file because its parent is named `state`, not `tailscale`. | Existing node identity remains at the same explicit state path; certificate and ACME material is stored beneath the already private/ignored state directory. Restart/reconfiguration with this argument fixed TLS. | Device log, upstream source inspection and successful retest, 2026-09-22 |
| D-022 | Treat the unauthenticated HTTPS 401 from `/next` as proof of transport/backend reachability only. | Safari successfully negotiated managed TLS through private Serve and the loopback proxy reached Page Turner's authentication boundary. | PWA hostname/443 support and coordinated authenticated Next/Back tests are now unblocked; no visible page turn or wake-lock outcome is implied by the 401. | User report, 2026-09-22 |
...
| 2026-09-22 | Owner enabled HTTPS in the Tailscale admin console and copied the revised configure/status scripts. Serve configured active and showed the expected endpoint/proxy mapping. |
| 2026-09-22 | iPhone traffic reached the Kindle TLS listener, but handshakes failed repeatedly with `no TailscaleVarRoot`. Upstream 1.102.4 source traced this to absent writable certificate storage. Added `--statedir=<extension>/state` to the persistent daemon launch. |
| 2026-09-22 | After restarting/reconfiguring with `--statedir`, Safari reached the private Serve `/next` endpoint and received the expected unauthenticated 401. Private TLS and loopback backend reachability are proven; authenticated PWA control remains pending. |
