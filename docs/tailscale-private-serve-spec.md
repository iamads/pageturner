...
| 2026-09-22 | iPhone traffic reached the Kindle TLS listener, but handshakes failed repeatedly with `no TailscaleVarRoot`. Upstream 1.102.4 source traced this to absent writable certificate storage. Added `--statedir=<extension>/state` to the persistent daemon launch; on-device retest pending. |
