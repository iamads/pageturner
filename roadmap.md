...
- Browser transport evidence: The deployed Kindle API is plain HTTP and initially had no CORS/OPTIONS handling. After narrowly scoped Pages-origin CORS/OPTIONS was installed, the owner reports the GitHub Pages PWA successfully controls the Kindle from desktop Chrome. The same app fails on the iPhone in Safari and Firefox Focus; Focus diagnostics show a secure browser context but an HTTP-Kindle fetch failing in 4 ms with `TypeError: Load failed`. Because all iOS browsers use WebKit, these are not independent transport engines. Matching Kindle transport logs and Safari Web Inspector output have not yet been supplied, so mixed-content versus iOS local-network permission is not conclusively separated.
...
- Research correction: [Chrome Local Network Access](https://developer.chrome.com/blog/local-network-access) documents permission-gated mixed-content exemptions for private-IP HTTP requests, rolling out in Chrome 142. Desktop success confirms that path for Chrome. It does not establish an equivalent WebKit exemption; the current iPhone failure makes Kindle HTTPS or a non-PWA scope change more likely if logs confirm WebKit blocks before reaching the plugin.
...
| H6 | A secure-context PWA can directly control the Kindle without a laptop/relay | Determines viable mobile architecture | On-device origin/TLS/local-network/CORS experiment, including installed Home Screen mode | Desktop Chrome direct control works after CORS; iPhone Safari/Focus HTTP fetch fails in ~4 ms. Exact WebKit failure layer pending logs |
...
- Current branch API is HTTP-only, bodyless authenticated POST next/back, with narrowly scoped Pages-origin OPTIONS/CORS. It still has no TLS, static hosting, or health endpoint. The deployed Kindle copy has not been updated, and a static PWA alone cannot yet be assumed to connect successfully.
...
- Logging/CORS experiment: Added token-safe connection/request/status/close diagnostics plus OPTIONS 204 and response headers restricted to origin `https://iamads.github.io`, routes `/next`/`/back`, POST, and Authorization. The owner installed it and reports successful PWA control from desktop Chrome, validating CORS and the authenticated POST path there. Actual POST authentication and reader guards remain required. The same HTTP endpoint fails from iPhone Safari/Firefox Focus before a useful browser response; plugin logs are pending. Local validation: 32 plugin tests, 9 network tests, and 6 Python client tests pass. Three optional real-socket tests could not run because local LuaSocket is unavailable.
...
- [ ] Correlate one iPhone attempt with Kindle `PageTurner: transport` logs and, if available, Safari Web Inspector. Also test harmless direct navigation to `http://<kindle-ip>:8088/next`: a visible 401 confirms phone LAN navigation without turning a page.
...
| 2026-09-19 | Recorded split browser result: desktop Chrome PWA control succeeds, iPhone Safari/Firefox Focus fails; retained feasibility gate | Owner reports working laptop control after CORS and supplied iPhone Focus diagnostics showing secure context/API availability but a 4 ms HTTP fetch failure. WebKit browsers share the same engine; Kindle logs/exact OS/Home Screen wake test remain missing | 2 | User |
