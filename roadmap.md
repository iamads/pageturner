...
| H8 | The owner can pair/authenticate without leaking the token or excessive setup | Repeated manual token transcription is unsafe and cumbersome | Test native-camera link opening, fragment removal, authenticated check, manual modal and session-token lifecycle | Native-camera replacement implemented locally; real-device scan/setup acceptance unknown |
...
| H11 | Kindle QR pairing can remove endpoint/token transcription without leaking credentials or issuing accidental commands | Determines whether setup is fast and safe enough for repeated reading sessions | Display a versioned URL QR plus route text, scan using the native camera, verify inline authentication without a turn, and test malformed/stale links and lifecycle | Owner reports the PWA scanner works poorly and approved its replacement; native-camera readability, handoff and lifecycle still need device evidence |
...
- 2026-09-22 replacement implemented locally: native-camera HTTPS link QR, fragment-only credentials cleared on arrival, authenticated `/connect` with inline status/retry, manual-entry modal, no PWA decoder/camera, service-worker cache v4. Existing lifecycle/transport/command/wake semantics retained. Local validation: 39 Lua plugin/API/pairing, 9 network, 6 Python client and 33 JS tests pass (87 total); three optional socket tests are skipped because local LuaSocket is unavailable. Device acceptance and deployment of this replacement remain pending; no phase transition.
...
- [ ] Validate QR readability/native-camera opening, browser/installed-context behavior, fragment removal, inline status, modal entry, token rotation and stale-token rejection on the actual Kindle/iPhone.
