...
- Confirmed revision (2026-09-22): Replace manual phone pairing as the primary flow with a Kindle-displayed QR code. Each manual Page Turner start creates a new in-memory bearer token, chooses active private Serve only when it maps to the current plugin port (otherwise local IPv4), and shows the route/endpoint below the QR. The PWA scans and validates the versioned payload without issuing a command; manual entry remains fallback.
...
| H8 | The owner can pair/authenticate without leaking the token or excessive setup | Repeated manual token transcription is unsafe and cumbersome | Test Kindle QR generation, PWA camera scan, manual fallback and session-token lifecycle | QR flow implemented with memory-only token handling; actual-device scan/setup acceptance unknown |
...
| H11 | Kindle QR pairing can remove endpoint/token transcription without leaking credentials or issuing accidental commands | Determines whether setup is fast and safe enough for repeated reading sessions | Generate a per-manual-start token, display a versioned QR plus route text, scan in the PWA, and test lifecycle/malformed/stale payloads | Plugin/PWA integration implemented and locally tested; Kindle QR readability, iPhone camera scan and lifecycle behavior untested |
...
4. Add QR pairing. On each manual Start, obtain 32 bytes from the OS random source, retain the encoded token only for that listener session (including normal suspend/resume), and invalidate it on manual Stop, book close or KOReader exit. Check active Serve with a bounded non-blocking operation and select it only when its proxy target matches the current plugin port; otherwise use current local IPv4. Display a versioned `{version, endpoint, token}` QR with route/endpoint text and a menu action to reopen it.
5. Add an explicit PWA camera action. Request rear-camera access only after a user gesture, decode with a bundled/local library rather than relying on inconsistent `BarcodeDetector` support, strictly validate the payload, populate connection state without sending a request, stop camera tracks after scan/cancel/background, and retain manual entry.
...
- Pairing evidence: QR generation/display result, selected route and endpoint, token lifecycle boundary, scan success/failure, time from Start to configured PWA, camera permission outcome, malformed/stale payload handling, and confirmation that scanning alone sends no request. Never log or screenshot the payload/token.
...
- P2-O4 (confirmed capability; revised measurement): Owner can install/launch the PWA on the target iPhone, scan the Kindle pairing QR, verify the displayed route/endpoint, and produce one correct next and back turn with no running laptop. Repeat after a new manual listener start to prove token rotation; verify the prior token and malformed/foreign QR payloads cannot configure valid control or turn pages. Manual endpoint/token entry remains a documented fallback.
...
- The KOReader API remains HTTP-only, bodyless authenticated POST next/back, with narrowly scoped Pages-origin OPTIONS/CORS and no health endpoint. Private Serve supplies TLS for the iPhone without changing the protocol. The PWA must retain both direct IPv4 and private Tailscale endpoint modes. QR pairing changes token provisioning and setup, not command semantics.
...
- 2026-09-22: QR pairing implemented locally. Decisions: per-manual-start in-memory token retained across normal suspend/resume; old laptop token may stop matching; automatic QR plus reopen menu; versioned JSON payload; PWA camera scan never sends a command; manual setup remains fallback. On-device evidence remains pending.
...
- [x] QR pairing direction confirmed: per-manual-start token, automatic and reopenable Kindle QR, versioned payload, PWA camera scanner, no scan-triggered command, manual fallback.
- [ ] Validate QR readability, camera permissions/scanning, token rotation and stale-token rejection on the actual Kindle/iPhone.
