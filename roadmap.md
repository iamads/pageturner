...
- Research conclusion for the tested devices: [Chrome Local Network Access](https://developer.chrome.com/blog/local-network-access) permits the desktop Chrome HTTPS→local-HTTP path. Correlated iPhone and Kindle logs show WebKit blocks the equivalent PWA fetch before it reaches the plugin even though direct HTTP navigation works. Retaining the secure-context PWA requirement therefore requires Kindle HTTPS for the next experiment; alternatives such as Shortcut/native control or relaxing wake-lock/PWA requirements are scope changes.
...
- [ ] Select next experiment: cross-origin Kindle HTTPS with existing Pages/CORS (least frontend change) versus same-origin Kindle-hosted HTTPS PWA (more plugin/static-serving work). Choose certificate/hostname setup first.
