...
- Confirmed: Phone + Kindle only for mobile operation; no running laptop bridge. Internet connectivity during reading is acceptable. A managed VPN/tunnel is under research but not selected; public exposure remains unapproved.
...
| H6 | A secure-context PWA can directly control the Kindle without a laptop/relay | Determines viable mobile architecture | On-device origin/TLS/local-network/CORS experiment, including installed Home Screen mode | HTTP variant fails: iPhone WebKit blocks Pages→Kindle fetch before network delivery; direct phone HTTP and desktop Chrome work. Kindle HTTPS untested |
...
| H9 | Current Tailscale can run acceptably on the owner's Kindle and provide Serve or Funnel HTTPS | Candidate avoids changing-IP certificate management while preserving secure PWA/wake lock | After owner-approved read-only hardware/kernel checks, run a reversible compatibility/resource/HTTPS proxy spike | Public/community precedent only; Kindle model/kernel unknown, nothing installed |
...
2. Plain HTTP is now disproven for the tested iPhone WebKit path. Investigate a stable browser-trusted HTTPS transport without manual IP certificates. Current candidate: Tailscale Serve (private; phone app required) or Funnel (public managed tunnel; explicit scope/security approval required). No running laptop bridge is allowed. Managed VPN/tunnel use remains a pending architecture decision, not a confirmed outcome revision.
...
- Unknown: Tailscale Serve versus Funnel, phone VPN acceptance, Kindle model/kernel/CPU/resource compatibility, public exposure, production frontend architecture, and token pairing/storage. Investigate before committing implementation architecture. Current comparisons: [Mobile PWA hosting strategies](docs/mobile-pwa-hosting-strategies.md) and [Tailscale research](docs/tailscale-research.md).
...
- [ ] Select Tailscale Serve (private; phone app required) versus Funnel (public endpoint; explicit guardrail revision/hardening required), or reject Tailscale after compatibility evidence.
- [ ] Record Kindle model, kernel, CPU architecture, memory/storage, and current-binary compatibility—but describe checks and obtain confirmation before inspecting the device.
- [ ] Decide whether installing/connecting the Tailscale iOS app is acceptable.
