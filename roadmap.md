...
- Browser transport risk: The deployed Kindle API is plain HTTP and initially had no CORS/OPTIONS handling. Chrome subsequently reported that its preflight response lacked `Access-Control-Allow-Origin`, proving a preflight response was received but not that the eventual POST/iPhone path works. The current branch implements narrowly scoped Pages-origin CORS/OPTIONS; device verification is pending. Exact local-network/security behavior must still be verified on the target iPhone.
...
- Research correction: [Chrome Local Network Access](https://developer.chrome.com/blog/local-network-access) documents permission-gated mixed-content exemptions for private-IP HTTP requests, rolling out in Chrome 142. The earlier inference that Kindle TLS was necessarily required is withdrawn. Scoped CORS plus local-network permission is now the active experiment before TLS. This is not target-iPhone evidence or approval to weaken authentication; Safari/iOS behavior remains unknown.
...
| H6 | A secure-context PWA can directly control the Kindle without a laptop/relay | Determines viable mobile architecture | On-device origin/TLS/local-network/CORS experiment, including installed Home Screen mode | Chrome identified rejected preflight; scoped CORS implemented locally. Kindle retest and target iPhone untested |
...
- Logging/CORS experiment: Added token-safe connection/request/status/close diagnostics. After Chrome explicitly reported a failed CORS preflight, added OPTIONS 204 and response headers restricted to origin `https://iamads.github.io`, routes `/next`/`/back`, POST, and Authorization. Actual POST authentication and reader guards remain required. No PWA or TLS change; not deployed to Kindle. Local validation: 32 plugin tests, 9 network tests, and 6 Python client tests pass. Three optional real-socket tests could not run because local LuaSocket is unavailable.
