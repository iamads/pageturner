...
- Confirmed for the spike: GitHub Pages may host the static PWA shell; it is not a command relay and contains no token. The public repository and Pages workflow deployed successfully, but inherited `abhijeet.de` routing prevents access to the artifact. Do not disrupt the existing root site; a dedicated subdomain/route or revised static host needs owner direction.
...
- 2026-09-19: Added the minimal `mobile-pwa/` feasibility harness before choosing a production architecture. It includes install metadata/offline shell caching, direct endpoint configuration, one-shot Next/Back requests, foreground wake-lock lifecycle controls, and token-redacted diagnostics. GitHub Pages deployed successfully, but the account's inherited custom-domain redirect makes the artifact unreachable; no existing site configuration was changed. No Kindle API/TLS/CORS behavior changed; reachable hosting and actual-iPhone combined connectivity/wake evidence remain the next checkpoints.
...
- [ ] Static spike hosting/bootstrap: GitHub Pages workflow deployed, but inherited `abhijeet.de` routing makes the artifact unreachable; choose a non-disruptive subdomain/route or revised host.
