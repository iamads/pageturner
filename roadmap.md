...
- 2026-09-19 desktop diagnostic: Chrome 151 on macOS loaded the Pages app as a secure browser context with Wake Lock and service-worker APIs available, but `fetch` from `https://iamads.github.io` to `http://192.168.0.102:8088` failed with `TypeError: Failed to fetch` after 2 ms. An independent non-mutating request from the same development machine reached that HTTP endpoint and received 401 in 284 ms, so basic network reachability and the listener are present; the browser security boundary, not the entered token, blocked this attempt before an application response. Exact DevTools policy text was not captured. This confirms the current HTTPS-Pages-to-HTTP-Kindle design does not work in the tested desktop browser; TLS plus subsequent CORS/private-network handling still require a selected design and target-iPhone evidence. H7 remains untested because wake lock was available but inactive.
...
| H6 | A secure-context PWA can directly control the Kindle without a laptop/relay | Determines viable mobile architecture | On-device origin/TLS/local-network/CORS experiment, including installed Home Screen mode | Current Pages HTTPS → Kindle HTTP attempt blocked in desktop Chrome; target iPhone and a direct HTTPS design remain untested |
...
| 2026-09-19 | Recorded that the first desktop browser request cannot cross from Pages HTTPS to the Kindle's current HTTP API; no phase transition or token failure inferred | Owner supplied redacted Chrome diagnostics showing a 2 ms fetch failure; independent HTTP probe reached the listener and received 401, isolating basic reachability from browser policy. Exact policy console text and iPhone behavior remain open | 2 | User supplied diagnostics; assistant verified listener reachability |
